...
impl RateLimiter {
    ...
    /// # Example
    /// ```
    /// // 100 messages capacity, 10 tokens/second (600 messages/minute)
    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        // Get user_id from JWT claims (already authenticated)
        let user_id = req
            .extensions()
            .get::<authz::jwt::Claims>()
            .map(|claims| claims.user_id.to_string())
            .unwrap_or_else(|| {
                // Fallback to IP if no claims (shouldn't happen after auth middleware)
                req.connection_info()
                    .realip_remote_addr()
                    .unwrap_or("unknown")
                    .to_string()
        ...
        let path = req.path().to_string();
        let key = format!("rate_limit:chat:{}:{}", user_id, path);
        ...
        Box::pin(async move {
            ...
            if !bucket.try_consume(1.0) {
                ...
                let _ = redis_cache.set(&key, &bucket, 300);
            ...
            // Save bucket state
            if let Err(e) = redis_cache.set(&key, &bucket, 300) {
