...
impl RateLimiter {
    ...
    /// # Example
    /// ```
    /// // 10 requests capacity, refill 10/60 tokens/second (10 requests/minute)
    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        // Extract user identifier from peer address (IP only, without port)
        let peer_ip = req.peer_addr()
            .map(|addr| addr.ip().to_string())
            .unwrap_or_else(|| "unknown".to_string());
        let key = format!("rate_limit:token_bucket:{}:{}", peer_ip, path);
        ...
        Box::pin(async move {
            ...
            if !bucket.try_consume(1.0) {
                ...
                tracing::warn!(
                    "Rate limit exceeded for {} on {} (tokens: {:.2})",
                    peer_ip, path, bucket.tokens
                );
            ...
            Ok(ServiceResponse::new(http_req, http_res).map_into_left_body())
        })
    }
}
